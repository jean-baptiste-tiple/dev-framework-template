...
Il est identifié par `/plan` pour tout produit MCP-first et installé par la story « Setup technique ».
...
| `schema-status.ts` | `src/lib/schemas/status.ts` | Schema Zod partagé = source de vérité (mcp-patterns §1) |
